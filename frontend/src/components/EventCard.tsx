    ...
    <div className={`card flex flex-col transition-all duration-300 hover:-translate-y-1 hover:border-amber-500/30 ${isSoldOut ? 'opacity-75' : ''}`}>
      {mediaUrl ? (
        <div className="border-b border-[#222] bg-black">
          <img
            src={mediaUrl}
            alt={`${event.title} promotional poster`}
      ...
      <div className="flex-1 p-5 sm:p-6">
        <div className="flex items-start justify-between gap-3 mb-4">
          <div>
            {event.type && (
              <span className="text-xs font-bold uppercase text-amber-500 mb-1 block">
                {event.type}
              </span>
            )}
            {event.coachName && (
      ...
      <div className="flex flex-col gap-3 border-t border-[#222] px-5 pb-5 pt-4 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:pb-6">
        <div>
          ...
          <span className="ml-2 text-xs font-semibold text-gray-500">full week</span>
          <p className="text-xs text-gray-500">Drop-in from $30/day</p>
        </div>
        {isSoldOut ? (
