...
export default function PageSkeleton({
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div className="space-y-3 rounded-2xl border border-[#1a1a1a] bg-[#0d0d0d] px-4 py-5 sm:px-6">
        <div className={`skeleton h-9 ${titleWidthClassName}`} />
        <div className="skeleton h-4 w-full max-w-xl" />
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {Array.from({ length: count }).map((_, index) => (
          <div key={index} className="rounded-2xl border border-[#222] bg-[#111] p-5 sm:p-6">
