...
export default function ProgramCard({ program, featured = false, variant = 'default' }: ProgramCardProps) {
  if (variant === 'compact') {
    return (
      <div className={`card p-6 flex flex-col h-full transition-all duration-300 hover:-translate-y-1 hover:border-amber-500/40 ${featured ? 'border-amber-500/30' : ''}`}>
        <div className="text-4xl mb-4">{program.icon}</div>
        <p className="text-amber-500 font-black text-xl mb-1">{program.priceLabel}</p>
        <h3 className="text-white font-black text-xl mb-3">{program.name}</h3>
        <p className="text-gray-400 text-sm leading-relaxed flex-1">{program.shortDescription}</p>
        <Link to="/book" className="btn-primary text-sm text-center mt-5">
          Book Now
        </Link>
      </div>
    )
  ...
  return (
    <div className={`card flex flex-col h-full transition-all duration-300 hover:border-amber-500/30 ${featured ? 'border-amber-500/40 ring-1 ring-amber-500/20' : ''}`}>
      {featured && (
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        {program.whoItsFor && (
          <p className="text-xs text-gray-500 border-t border-[#222] pt-4 mb-5 italic">
            {program.whoItsFor}
          </p>
        )}

        <Link to="/book" className="btn-primary text-center text-sm">
          Book {program.name}
        </Link>
      </div>
    </div>
