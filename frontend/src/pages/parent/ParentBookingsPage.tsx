    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-2">
            View upcoming sessions, review past bookings, and cancel if plans change.
          </p>
        </div>
        <Link
          to="/book"
      ...
      {filteredBookings.length === 0 ? (
        <EmptyState
          ...
          description={filter === 'upcoming' ? 'You do not have any upcoming sessions yet.' : 'No bookings match this view.'}
          action={
            <Link to="/book" className="bg-green-500 text-black font-bold px-5 py-2 rounded-lg text-sm">
              Book a Session
            </Link>
          }
        />
              ...
              <div key={booking.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div>
                    ...
                    {booking.parentName ? (
                      <p className="text-gray-500 text-sm">Parent: {booking.parentName}</p>
                    ) : null}
                    {booking.notes ? (
                  ...
                  <div className="flex gap-2">
                    ...
                    <Link
                      to="/book"
                      className="text-sm text-green-400 border border-green-400/30 hover:bg-green-400/10 rounded-lg px-4 py-2"
                    >
                      Book Again
                    </Link>
                  </div>
