...
export default function AdminCoachesPage() {
  ...
  // Users who don't already have a coach profile
    ...
    <div>
      ...
      {showAdd && (
        <form onSubmit={handleCreate} className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-6 space-y-4">
          <h2 className="text-white font-bold text-lg">Create Coach Profile</h2>
          <div>
          ...
          <div className="flex gap-3">
            <button
              type="submit"
              disabled={saving}
              className="bg-green-500 hover:bg-green-600 text-white font-semibold px-4 py-2 rounded-lg text-sm transition-colors disabled:opacity-50"
            >
      ...
      {/* Coaches Table */}
      {coaches.length === 0 ? (
        <div className="text-gray-500 text-center py-12">No coach profiles yet.</div>
      ) : (
            ...
            >
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-xs ${c.active ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
